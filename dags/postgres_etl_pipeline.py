...
with DAG(
    ...
    def create_table():
        ...
        hook = PostgresHook(postgres_conn_id="market_postgres_database_conn")
        #  Query to create the market_data table if it does not exist
    ...
    def load_market_data(data_records):
        # Pull the connection
        market_database_hook = PostgresHook(
            postgres_conn_id="market_postgres_database_conn"
        )
        market_database_conn = market_database_hook.get_connection(
            market_database_hook.postgres_conn_id
        )
        market_database_engine = create_engine(
            f"postgresql+psycopg2://{market_database_conn.login}:{market_database_conn.password}@{market_database_conn.host}:{market_database_conn.port}/{market_database_conn.schema}"
        )
        ...
        if data_records[0]["status"] == "OK":
            flattened_dataframe.to_sql(
                name="market_data",
                con=market_database_engine,
                if_exists="append",
                index=False,
            )
        else:
            pass  # Handle the case where the API response is not OK
    ...
    create_table() >> load_market_data(transformed_market_data)
